...
# Definiciones de estructuras y callbacks nativos
...
# Instancia estática del callback y la vtable simulada
...
class IDRS15Engine:
    ...
    def initialize(self) -> bool:
        ...
        try:
            # Añadir directorio al PATH de DLLs de Windows
    ...
    def _setup_prototypes(self):
        """Configura los prototipos exactos de funciones nativas."""
        if not self.ocr_dll:
            return
        ...
        self.drs_set_image = self.ocr_dll.drs_set_image
        self.drs_set_image.restype = ctypes.c_int
        self.drs_set_image.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_int, ctypes.c_int, ctypes.c_int]
        ...
        self.drsOcr.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
        ...
        if self.drs_get_nb_zones:
            ...
            self.drs_get_nb_zones.argtypes = [ctypes.c_void_p]
    ...
    def _create_and_prep_engine(self) -> Optional[ctypes.c_void_p]:
        """Crea una nueva instancia nativa del motor iDRS15 y aplica los parches de seguridad."""
        ...
        # 1. Inyectar mock logger en [engine + 0x5058]
        try:
        ...
        # 2. Configurar directorio de recursos OCRResources
        ...
        # 3. Configurar alfabeto y opciones de salida
        self.drs_set_alphabet(handle, 1)  # 1 = Latín
        self.drs_set_output_retn(handle, 1)

        # 4. Establecer flag de entorno inicializado (0x50ea = 1)
        try:
    ...
    def process_image_bytes(self, pixel_bytes: bytes, width: int, height: int, pitch: int) -> Tuple[bool, int]:
        """Efectúa OCR sobre un búfer de imagen en memoria."""
        ...
        try:
            ...
            set_res = self.drs_set_image(handle, img_ptr, width, height, pitch)
            if set_res != 0:
                return False, set_res

            ocr_res = self.drsOcr(handle, 1)
            zones = 0
            if self.drs_get_nb_zones:
                zones = self.drs_get_nb_zones(handle)

            return ocr_res >= 0 or zones >= 0, zones
        finally:
            self.drsD_destroy_drs(handle)

    def process_pdf(self, src: Path, dst: Path, lang: str = "spa+eng", dpi: int = 300) -> Tuple[bool, str]:
        """
        Procesa un archivo PDF usando el motor nativo iDRS15.
        Convierte las páginas del PDF a imágenes y aplica el motor OCR.
        """
...
# Función de compatibilidad y diagnóstico
